...
export default function LibraryPage() {
  ...
  const filtered = useMemo(
    () => filterOwnedBooks(books ?? [], filter, search),
    [books, filter, search],
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4 border-l-2 border-gold pl-4">
        <div>
          ...
          <p className="page-subtitle">Volumes in your possession</p>
        </div>
        <Link href="/books/new?shelf=owned" className="text-sm px-3 py-1.5 rounded-md btn-primary">
          Add volume
        </Link>
      </div>
      <input
        ...
        {filters.map((f) => (
          <button
            ...
            className={`px-3 py-1 rounded-md text-sm ${
              filter === f.value ? "chip-active" : "chip-inactive"
            }`}
          >
            {f.label}
          </button>
      ...
      {isLoading ? (
        <p className="text-parchment-muted italic">Loading shelves…</p>
      ) : (
        <BookList
          books={filtered}
          ...
          renderActions={(book) => (
            <select
              value={book.read_status ?? "unread"}
              onChange={(e) =>
                updateReadStatus.mutate({
                  id: book.id,
                  status: e.target.value as "unread" | "reading" | "read",
                })
              }
